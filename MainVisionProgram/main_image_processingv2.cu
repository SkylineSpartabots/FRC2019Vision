...
Scalar hsv_min(21,0,144);
Scalar hsv_max(95,98,255);
const int minArea = 350;
...
int main(int argc, char** argv)
{
	...
	for (; ; )
	{
		...
		if(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count() - lastImageSentTime > 1000/fps)	{
			//video.write(frame);
			lastImageSentTime = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
		}
		newImage = true;
	}
}
