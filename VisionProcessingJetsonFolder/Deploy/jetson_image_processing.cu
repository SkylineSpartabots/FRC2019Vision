...
const int sizeX = 960; //960
const int sizeY = 544; //544
const int fps = 15;

//TODO: String formatter
//Driver Station at 10.29.76.212 IP
const string STREAM_STRING = "appsrc ! videoconvert ! video/x-raw, format=(string)I420, width=(int)960, height=(int)544, framerate=15/1 ! omxh264enc bitrate=600000 ! video/x-h264, stream-format=(string)byte-stream ! h264parse ! rtph264pay ! udpsink host=10.0.0.60 port=5801 sync=true ";
const string DEBUG_STRING = "appsrc ! videoconvert ! video/x-raw, format=(string)I420, width=(int)960, height=(int)544, framerate=15/1 ! omxh264enc bitrate=600000 ! video/x-h264, stream-format=(string)byte-stream ! h264parse ! rtph264pay ! udpsink host=10.0.0.60 port=5802 sync=true ";
VideoWriter debug;
//const Mat camera_matrix = (cv::Mat_<float>(3,3) << 786.42, 0, 297.35, 0 , 780.45, 214.74, 0, 0, 1);
const Mat camera_matrix = (cv::Mat_<float>(3,3) << 840.421, 0, 487.9345, 0 , 837.579, 255.956, 0, 0, 1);
//const Mat dist_coeffs = (cv::Mat_<float>(1,5) <<  0, 0,  0, 0, 0);
const Mat dist_coeffs = (cv::Mat_<float>(1,5) <<  0.11743, -1.19676,  -0.009739, -0.001417, 2.0805);
		...
		if(target.targetType == 1) {
  		...
  		//TermCriteria criteria = TermCriteria(CV_TERMCRIT_EPS + CV_TERMCRIT_ITER, 40, 0.001 );
			//if(image_points.size() > 0)	{
				//cv::cornerSubPix(grayFrame, image_points, winSize, zeroZone, criteria );
			//}
...
int main(int argc, char** argv)
{
	...
	system (setting_script); //TODO for chessboard
